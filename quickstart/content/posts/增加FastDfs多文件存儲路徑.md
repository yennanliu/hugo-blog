...
draft = false
+++
<!--more-->項目需要增加聊天會話功能，涉及到上傳語音圖片等信息。考慮新增一個目錄，所有相關文件存在一個相同的目錄中。因此需要對原項目增加一個存儲的路徑。以前的項目因爲只有一個路徑，且已經運行中。走了些彎路，僅此記錄操作過程。nginx version 1.16.1,fastdfs-nginx-module
1.首先在 /etc/fdfs/storage.conf配置中增加文件路徑。
